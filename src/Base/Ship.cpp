...
//	atPosition.y += levelEntity->worldPosition
	...
	{
		pp->velocity = PlayingLevelRef().GetLevel().BaseVelocity();
...
void Ship::Process(PlayingLevel& playingLevel, PlayerShip* playerShip, int timeInMs)
{
	...
	if (movements.Size())
		movements[currentMovement].OnFrame(playerShip, timeInMs);
	// AI
...
void Ship::ProcessAI(PlayerShip* playerShip, int timeInMs)
{
	...
	rota.OnFrame(playerShip, timeInMs);
...
bool Ship::SwitchToWeapon(int index)
{
	...
	LogMain("Switched to weapon : "+activeWeapon->name, DEBUG);
//	UpdateStatsFromGear();
	// Update ui
	if (!enemy)
	{
